...
spark-submit --master yarn --num-executors $1 --executor-memory 4g --executor-cores 2 --class MatrixMultiply inner-matrix-multiply.jar $2 $3 $4$i $5 $6 $7
fin=`date +%s`
printf "$(date) $line\n"
runtime=$((fin-begin))
printf "Execution time: $runtime\n\n"
hdfs dfs -cat /innerProductResult/* > innerProductResult"$(($i+1))"
hdfs dfs -rm -r /innerProductResult
hdfs dfs -put innerProductResult"$(($i+1))" /
rm innerProductResult"$(($i+1))"
done

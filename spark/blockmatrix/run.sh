...
#spark-submit --num-executors <num_executors> --class MatrixMultiply --master yarn with_args.jar <num_partitions> <left_input_matrix_directory> <right_input_matrix_directory> <left_matrix_row_size> <left_mat_col/right_mat_row_size> <right_matrix_col_size> <mPerBlock> <kPerBlock> <nPerBlock>
...
printf "$(date) $line\n"
begin=`date +%s`
spark-submit --master yarn --num-executors $1 --class MatrixMultiply block-matrix-multiply.jar $2 $3 $4 $5 $6 $7 $8 $9 $10
fin=`date +%s`
printf "$(date) $line\n"
runtime=$((fin-begin))
